"""
Демо-дані: одна основна школа з 1000 учнями (паралелі А1–А11, Б1–Б11 та 12-А),
директор, два завучі (паралель А та Б), чотири STEM-предметники (S/T/E/M) із
розподілом класів через teacher_class_assignments за циклом (кожен клас має
«свого» предметника по черзі). Класні керівники лишаються на classes.teacher_id
без stem_specialty — це лише класне керівництво.
...
def main():
    ...
    ins_user = text(
        """
        INSERT INTO users (id, email, password_hash, full_name, role, school_id, staff_scope, stem_specialty, email_verified)
        VALUES (CAST(:id AS uuid), :email, :hash, :name, :role, CAST(:sid AS uuid), :staff_scope, :stem_specialty, TRUE)
        ON CONFLICT (id) DO UPDATE SET
            ...
            stem_specialty = EXCLUDED.stem_specialty,
            email_verified = TRUE
        """
    )

    ins_teacher_assign = text(
        """
        INSERT INTO teacher_class_assignments (user_id, class_id)
        VALUES (CAST(:uid AS uuid), CAST(:cid AS uuid))
    ...
    with engine.begin() as conn:
        for row in (
            {"id": sch1, "name": "ЗЗСО №1 «STEM-демо»", "city": "Київ"},
            {"id": sch2, "name": "ЗЗСО №2 (тест мережі)", "city": "Львів"},
        ):
            conn.execute(
                text(
                    """
        ...
        def parallel_slug_for_teacher_email(letter: str) -> str:
            """У класі в БД назва кирилична (7-А), у email класного — латиниця A/B (зручно з англ. розкладки)."""
            m = {"А": "A", "Б": "B"}
        ...
        for ci, (gr, letter) in enumerate(class_specs, start=1):
            ...
            cname = f"{gr}-{letter}"
            email_slug = f"{gr}-{parallel_slug_for_teacher_email(letter)}"
            conn.execute(
                ins_user,
                {
                    "id": t_id,
                    "email": f"teacher.s1.{email_slug}@demo.stem.local",
                    "hash": pwd_hash,
        ...
        conn.execute(
            text(
                """
                DELETE FROM teacher_class_assignments AS tca
                USING classes AS c
                WHERE tca.class_id = c.id AND c.school_id = CAST(:sid AS uuid)
                """
            ),
            {"sid": sch1},
        )
        conn.execute(
            text(
                """
                UPDATE users SET stem_specialty = NULL
                WHERE school_id = CAST(:sid AS uuid)
                  AND role = 'user'
                  AND (
                      staff_scope IS NOT NULL
                      OR email LIKE '%teacher.s1.%@demo.stem.local'
                  )
                """
            ),
            {"sid": sch1},
        ...
        stem_demo_specs = [
            ("S", uid("stem_subject", "1", "S"), "teacher.stem.S.s1@demo.stem.local", "STEM-предметник — Наука (S)"),
            ("T", uid("stem_subject", "1", "T"), "teacher.stem.T.s1@demo.stem.local", "STEM-предметник — Технології (T)"),
            ("E", uid("stem_subject", "1", "E"), "teacher.stem.E.s1@demo.stem.local", "STEM-предметник — Інженерія (E)"),
            ("M", uid("stem_subject", "1", "M"), "teacher.stem.M.s1@demo.stem.local", "STEM-предметник — Математика (M)"),
        ]
        stem_tid_by_letter = {}
        for stem_letter, st_id, semail, sname in stem_demo_specs:
            stem_tid_by_letter[stem_letter] = st_id
            conn.execute(
                ins_user,
                {
                    "id": st_id,
                    ...
                    "sid": sch1,
                    "staff_scope": None,
                    "stem_specialty": stem_letter,
                },
            )

        for ci, (gr, letter) in enumerate(class_specs):
            c_id = uid("class", "1", str(gr), letter)
            sl = STEM_ROT[ci % 4]
            conn.execute(
                ins_teacher_assign,
                {"uid": stem_tid_by_letter[sl], "cid": c_id},
            )
        ...
        for cid in (
            ...
            conn.execute(ins_teacher_assign, {"uid": manual_tea_id, "cid": cid})
    ...
    print(
        "OK: школа 1 — 1000 учнів у класах 1-А…11-А, 1-Б…11-Б та 12-А; директор + 2 завучі; "
        "4 STEM-предметники (S/T/E/M) із черговим призначенням на класи через teacher_class_assignments; "
        "школа 2 — лише директор."
    )
    ...
    print(
        "                                      (демо-предметник: STEM «M», 13 призначень до класів 5–12; "
        "класних учителів не замінює)"
    )
    print("  STEM-предметники (школа 1, пароль Demo2026!):")
    print("    teacher.stem.S.s1@demo.stem.local — Наука (S)")
    print("    teacher.stem.T.s1@demo.stem.local — Технології (T)")
    print("    teacher.stem.E.s1@demo.stem.local — Інженерія (E)")
    print("    teacher.stem.M.s1@demo.stem.local — Математика (M)")
    print("    (кожен клас має одного предметника за циклом S→T→E→M по порядку класів)")
    ...
    print("  Класний 7-А: teacher.s1.7-A@demo.stem.local (у частині «s1.…» паралель A/B латиницею)")
